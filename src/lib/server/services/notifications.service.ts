...
class NotificationsService {
    ...
    public async markAsRead(notificationId: string): Promise<void> {
        ...
        sseEmitter.emit(AppEvents.NotificationRead, notification.userId, { notificationId: notification.id })
    }

    public async sendTransactionConfirmedNotification(
        payload: EventMap[AppEvents.TransactionConfirmed]
    ): Promise<void> {
        const meta: NotificationMeta[NotificationType.Info] = { type: NotificationType.Info }
