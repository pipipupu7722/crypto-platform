...
import { Button, Col, DatePicker, Descriptions, Form, Input, List, Row, Select, Spin, Tag, Upload, message } from "antd"
...
const DocumentsTab = ({ initialUser }: { initialUser: User }) => {
    ...
    const [uploadedFiles, setUploadedFiles] = useState<{ id: string; name: string; path: string; status: string }[]>([])
    ...
    useEffect(() => {
        const fetchUploadedData = async () => {
            try {
                const response = await fetch(`/api/cabinet/documents?userId=${initialUser.id}`)
                if (!response.ok) {
                    throw new Error("Не удалось загрузить список документов")
                }
                const result = await response.json()

                const documents = result.documents.filter((doc: { type: string }) => doc.type === "ID")
                const selfies = result.documents.filter((doc: { type: string }) => doc.type === "SELFIE")
    ...
    const handleSave = async () => {
        ...
        await updateUserDetails(initialUser.id, {
            ...
            .then((res) => res.success ?? message.success("Данные пользователя успешно обновлены!"))
    ...
    return (
        <div>
            ...
            >
                ...
                <Row gutter={16}>
                    <Col xs={24} sm={24} md={12}>
                        ...
                        <Spin spinning={isActionPending}>
                            <Upload.Dragger {...draggerProps("ID")} style={{ height: 150 }}>
                                ...
                                <p className="ant-upload-text">Перетащите документы сюда или нажмите</p>
                                <p className="ant-upload-hint">Загрузите документы. Формат: jpg, png, pdf.</p>
                            </Upload.Dragger>
                        </Spin>
                    </Col>
                    ...
                    >
                        ...
                        <Spin spinning={isActionPending}>
                            <Upload.Dragger {...draggerProps("SELFIE")} style={{ height: 150 }}>
                                <p className="ant-upload-drag-icon">
                                    <UploadOutlined />
                                </p>
                                <p className="ant-upload-text">Перетащите селфи сюда или нажмите</p>
                                <p className="ant-upload-hint">Загрузите селфи. Формат: jpg, png.</p>
                            </Upload.Dragger>
                        </Spin>
                    </Col>
                </Row>
            ...
            <div style={{ textAlign: "center", marginTop: 20 }}>
                <Button type="primary" loading={isActionPending} onClick={handleSave} disabled={isActionPending}>
                    Сохранить файлы
                </Button>
            </div>
        </div>
