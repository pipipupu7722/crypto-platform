...
import {
	...
	ManagerDetailsSchemaType,
} from "@/schemas/dashboard/manager.schemas";

export const createManager = wrapsa(
	async (userDetails: ManagerDetailsSchemaType) => {
...
export const updateManagerDetails = wrapsa(
	async (userId: string, userDetails: ManagerDetailsSchemaType) => {
		await ProtectSa([UserRole.ADMIN]);
		...
		return await usersService.update(
			userId,
			ManagerDetailsSchema.parse(userDetails),
		);
	},
);
